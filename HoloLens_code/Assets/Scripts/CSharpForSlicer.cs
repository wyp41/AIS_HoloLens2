...
public class CSharpForSlicer : MonoBehaviour
{
    Thread mThread;
    ...
    Vector4 currentRot = Vector4.zero;
    ...
    private void Update()
    {
        ...
        currentRot[0] = transform.rotation.x;
        currentRot[1] = transform.rotation.y;
        currentRot[2] = transform.rotation.z;
        currentRot[3] = transform.rotation.w;
    }
    ...
    void SendAndReceiveData()
    {
        ...
        if (dataReceived != null)
        {
            ...
            else if (dataReceived.StartsWith("e"))
            {
                ...
                receivedRot = StringToQuaternion(dataReceived.Remove(0,1));
                calibed = true;
                cmd = true;
            }
            ...
            byte[] myWriteBuffer = Encoding.ASCII.GetBytes(currentPos.ToString().Remove(currentPos.ToString().Length-1, 1) + "," + currentRot.ToString().Remove(0,1)); //Converting string to byte data
    ...
    public static Quaternion StringToQuaternion(string sVector)
    {
        ...
        // store as a Vector3
        Quaternion result = new Quaternion(float.Parse(sArray[3]), float.Parse(sArray[4]), float.Parse(sArray[5]), float.Parse(sArray[6]));
